$(function () {
    ...
    $('.menu-btn').click(function () {
        if (open) {
            ...
            header.animate({ left: 0, width: '100%' });
            content.animate({ left: 0, width: '100%' });
            boxUsuario.hide().animate({ display: 'none' })
        } else {
            menu.show().animate({ width: '20%' });
            header.animate({ left: '20%', width: '80%' });
            content.animate({ left: '20%', width: '80%' });
            boxUsuario.show().animate({ display: 'block' })
        }
        open = !open;
    });
